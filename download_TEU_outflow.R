...
direction <- "OUT"                                                            # select the direction: "IN", "OUT", or "TOTAL"
...
for (i in 1:length(time)) {                                                   # loop over all reporter
  ...
  if (i==1){
    port_data_TEU_out <- x                                                                  # save and append to previous tables
  } else {
    port_data_TEU_out <- rbind(port_data_TEU_out, x)
  }
}



port_data_TEU_out <- pivot_wider(port_data_TEU_out, values_from = "TEU", names_from = "loadstat") %>%
...
for (i in 1:nrow(missing_entries2)) {
  port_data_TEU_out$port[port_data_TEU_out$rep_mar==missing_entries2$rep_mar[i]] <- missing_entries2$port[i]
  port_data_TEU_out$aggregate[port_data_TEU_out$rep_mar==missing_entries2$rep_mar[i]] <- missing_entries2$aggregate[i]
  port_data_TEU_out$stat_port[port_data_TEU_out$rep_mar==missing_entries2$rep_mar[i]] <- missing_entries2$stat_port[i]
  port_data_TEU_out$port_code[port_data_TEU_out$rep_mar==missing_entries2$rep_mar[i]] <- missing_entries2$rep_mar[i]
}
...
for (i in 1:nrow(incorrect_aggregates)) {
  port_data_TEU_out$aggregate[port_data_TEU_out$port_code==incorrect_aggregates$port_code[i]] <- incorrect_aggregates$aggregate[i]
  port_data_TEU_out$stat_port[port_data_TEU_out$port_code==incorrect_aggregates$port_code[i]] <- incorrect_aggregates$stat_port[i]
}

save(list = "port_data_TEU_out", file = "downloaded_data_TEU_out.RData")  
...
port_data_TEU_out <- merge(port_data_TEU_out, gdp_data, by.x = c("reporter_code", "year", "quarter"), by.y = c("geo", "year", "quarter"), all.x = TRUE)

######## ADD INFOMATION ON PORT GROUPS ########
average_TEU <- port_data_TEU_out %>% group_by(port_code, year) %>% summarize(TEU_yrly=sum(total_TEU, na.rm = T)) %>% ungroup() %>%
...
port_data_TEU_out <- merge(port_data_TEU_out, average_TEU, by.x = "port_code", by.y = "port_code", all.x = T)
...
port_data_TEU_out <- merge(port_data_TEU_out, treatment, by.x = c("port_code", "year", "quarter"), by.y = c("port_code", "year", "quarter"), all.x = T) %>% 
...
port_data_TEU_out$operation[port_data_TEU_out$port=="Rotterdam" & port_data_TEU_out$year<2016] <- 0 # Operations in Rotterdam were taken over since 2016
port_data_TEU_out$group_operation[port_data_TEU_out$port=="Rotterdam"] <- 80  # operation started Sept. 2016 

port_data_TEU_out <- port_data_TEU_out[!is.na(port_data_TEU_out$port),]       #delete remaining observations that are not associated with an port

######## SAVE ############

save(list = "port_data_TEU_out", file = "port_data_TEU_out.RData")                                # save in current directory

rm(list = setdiff(ls(), "port_data_TEU_out"))                                             #delete all helper objects
