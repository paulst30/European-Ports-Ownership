...
note <- ggdraw() + geom_text(aes(label="Note: Blue dots are ports that process Chinese containers."), x = 0.05, y = 0, hjust = 0,vjust = -1, size = 3, family="LM Roman 10")

plot_grid(distribution_ports, note, ncol = 1, rel_heights = c(0.95,0.05))
...
latex_code <- gsub("vars & mean.x & agrate & mean.y & agrate0 & agrate1"," & \\\\multicolumn{2}{c}{Control group} & \\\\multicolumn{3}{c}{Treatment group} \\\\\\\\\n \\\\cmidrule(lr){2-3} \\\\cmidrule(lr){4-6}  Var. & mean & $\\\\Delta$ & mean & $\\\\Delta$ [$t \\\\leq t^*$] & $\\\\Delta$ [$t>t^*$]", latex_code)
