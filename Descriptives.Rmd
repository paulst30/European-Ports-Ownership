...
through_put_by_ports <- port_data %>% filter(year>=2010) %>% mutate(china = ((sender == "Hong Kong" | sender == "China (except Hong Kong)") &                                          container>0)) %>%
                        group_by(port) %>% summarise(throughput=sum(container, na.rm=T), china=max(china)) %>% ungroup() %>%
