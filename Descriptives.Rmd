...
sumstat_controls <- port_data_agg %>% group_by(port_id) %>% mutate(treated=max(ownership_china)) %>% ungroup() %>%
                                      filter(treated==0, container!=0, !is.na(quay_length) & !is.na(cranes) & !is.na(max_lift) & !is.na(cargo_depth)) %>% 
                                      arrange(port_id, period) %>%
                                      group_by(port_id) %>%
                                      summarize(across(c("container", "quay_length", "cranes", "max_lift", "cargo_depth"), list(agrate = ~ (tail(.x,n=1)-head(.x,n=1))/(tail(period,n=1)-head(period,n=1)))),
                                                across(c("container", "quay_length", "cranes", "max_lift", "cargo_depth"), ~ mean(.x, na.rm=T), .names="{col}_mean")) %>% 
                                      ...
                                      pivot_longer(cols = starts_with(c("container_", "quay_length_", "cranes_", "max_lift", "cargo_depth_")), 
...
sumstat_treated <- port_data_agg %>% group_by(port_id) %>% mutate(treated=max(ownership_china))  %>%
                                      ...
                                      mutate(across(c("container", "quay_length", "cranes", "max_lift", "cargo_depth"), ~ mean(.x, na.rm=T), .names="{col}_mean")) %>%
                                      group_by(port_id, ownership_china) %>%
                                      summarize(across(c("container", "quay_length", "cranes", "max_lift", "cargo_depth"), list(agrate = ~ (tail(.x,n=1)-head(.x,n=1))/(tail(period,n=1)-head(period,n=1)))),
                                                across(c("container_mean", "quay_length_mean", "cranes_mean", "max_lift_mean", "cargo_depth_mean"), ~ max(.x))) %>%
                                      ...
                                      pivot_longer(cols = starts_with(c("container_", "quay_length_", "cranes_", "max_lift_", "cargo_depth_")), 
                                                   names_to = c("vars", ".value"),
                                                   names_pattern = "(.*)_(.*)")
