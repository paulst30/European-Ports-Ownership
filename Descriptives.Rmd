...
sumstat_controls <- port_data_agg  %>% group_by(port_id) %>% mutate(treated=max(ownership_china)) %>% ungroup() %>%
                                      ...
                                      group_by(port_id) %>%
                                      summarize(across(c("container", "share", "quay_length", "cargo_depth", "local_markets", "foreign_markets"), list(agrate = ~ (tail(.x,n=1)-head(.x,n=1))/(tail(period,n=1)-head(period,n=1)))),
                                                across(c("container", "share", "quay_length", "cargo_depth", "local_markets", "foreign_markets"), ~ mean(.x, na.rm=T), .names="{col}_mean")) %>%
                                      ungroup() %>% select(-port_id) %>%
                                      summarize(across(ends_with("mean"), mean, .names = "{col}"),
                                                across(ends_with("grate"),~ mean(.x,na.rm=T), .names = "{col}")) %>% 
                                      pivot_longer(cols = starts_with(c("container_", "share_", "quay_length_", "cargo_depth_", "local_markets_", "foreign_markets_")), 
...
sumstat_treated <- port_data_agg %>%  group_by(port_id) %>% mutate(treated=max(ownership_china))  %>%
                                      filter(treated==1, container!=0, port_code!="DEHAM") %>% 
                                       merge(share, 
                                            ...
                                            all.x = T) %>%
                                      arrange(port_id, period) %>%
                                      mutate(across(c("container","share", "quay_length", "cargo_depth", "foreign_markets", "local_markets"), ~ mean(.x, na.rm=T), .names="{col}_mean")) %>%
                                      group_by(port_id, ownership_china) %>%
                                      summarize(across(c("container","share", "quay_length", "cargo_depth", "foreign_markets", "local_markets"), list(agrate = ~ (tail(.x,n=1)-head(.x,n=1))/(tail(period,n=1)-head(period,n=1)))),
                                                across(c("container_mean","share_mean", "quay_length_mean", "cargo_depth_mean", "foreign_markets_mean", "local_markets_mean"), ~ max(.x))) %>%
                                      ...
                                      pivot_longer(cols = starts_with(c("container_", "share_", "quay_length_", "cargo_depth_", "foreign_markets_", "local_markets_")), 
                                                   names_to = c("vars", ".value"),
                                                   names_pattern = "(.*)_(.*)")
...
latex_code <- gsub("vars & mean.x & agrate & mean.y & agrate0 & agrate1"," & \\\\multicolumn{2}{c}{Control group [N=108]} & \\\\multicolumn{3}{c}{Treatment group [N=18]} \\\\\\\\\n \\\\cmidrule(lr){2-3} \\\\cmidrule(lr){4-6}  Var. & mean & $\\\\Delta$ & mean & $\\\\Delta$ [$t \\\\leq t^*$] & $\\\\Delta$ [$t>t^*$]", latex_code)
