...
#font_import(pattern="LM Roman 10")
...
port_data_TEU <- filter(port_data_TEU, aggregate==0, stat_port==1) %>%
                 mutate(port = ifelse(port=="Antwerpen", "Antwerp", port))
...
```{r, include=FALSE}
port_data_TEU <- filter(port_data_TEU, TEU_yrly >=20000) #exclude port that have an average yearly container throughput above 20000
...
#reduce sample to relevant ports (data availability + no colonies)
sample <- c("BEANR", "BEGNE", "BEZEE", "BGVAR", "CYLMS", "DEBRV", "DECUX", "DEHAM", "DELBC", "DEWVN", "DKAAL", "DKAAR", "DKCPH", "DKEBJ", "DKFRC", "EETLL", "ESAGP", "ESALC", "ESALG", "ESBCN", "ESBIO", "ESCAD", "ESCAR", "ESCAS", "ESGIJ",  "ESLPA", "ESMLN", "ESMPG", "ESPMI", "ESSCT", "ESSDR", "ESSVQ", "ESTAR", "ESVGO", "ESVIL", "ESVLC", "FIHEL", "FIHKO", "FIOUL", "FIPOR", "FIRAU", "FRBES", "FRDKK", "FRLEH", "FRMRS", "FRNTE", "GBBEL", "GBBRS", "GBCDF", "GBFOR", "GBFXT", "GBGOO", "GBHUL", "GBIMM", "GBLIV", "GBLON", "GBMED", "GBMME", "GBPME", "GBSOU", "GBWPT", "GRPIR", "GRSKG", "HRPLE", "HRRJK", "IEDRO", "IEDUB", "IEORK", "IEWAT", "ITAOI", "ITBRI", "ITCAG", "ITCTA", "ITCVV", "ITGIT", "ITGOA", "ITLIV", "ITMDC", "ITNAP", "ITRAN", "ITSAL", "ITSPE", "ITSVN", "ITTAR", "ITTRS", "ITVCE", "LTKLJ", "LVRIX", "MEBAR", "MTMAR", "NLAMS", "NLMOE", "NLRTM", "NOKRS", "NOMJF", "NOMSS", "NOOSL", "PLGDN", "PLGDY", "PLSZZ", "PTCNL", "PTFNC", "PTLEI", "PTLIS", "PTPDL", "PTPRV", "PTSET", "PTSIE", "ROCND", "SEGOT", "SEGVX", "SEHAD", "SEHEL", "SEMMA", "SENRK", "SESOE", "SEVST", "SIKOP", "TRAMB", "TRAYT", "TRGEM", "TRISK", "TRIZM", "TRIZT", "TRMER", "TRSSX", "TRTEK")
 ...
 port_data_agg <- filter(port_data_agg, port_data_agg$port_code %in% sample)
...
TENtec <- read.csv(file.path(getwd(),"control variables_new", "TENtec_corridors.csv"))       # on port_code level (check)
...
treated_ports <- port_data_TEU %>% filter(group!=0 & port_code!="DEHAM" & period<=105) %>% 
...
for (i in 1:length(treated_ports)){
  ...
  plot_list[[i]] <- port_data_TEU %>% filter(port_code==treated_ports[i] | group==0, period<=105) %>% group_by(port_code, time) %>%
...
ggsave(width=17, height=10, filename = "port_ts_graph_presentation.pdf", path = file.path(getwd(), "presentation_results"))
...
ggsave(width=12, height=8, filename = "port_ts_graph_presentation_6.pdf", path = file.path(getwd(), "presentation_results"))
...
#controls
share <- port_data_TEU %>% filter(port_code!="DEHAM") %>% mutate(china = (sender_code=="CN_X_HK")) %>% group_by(port_code, china, period) %>%
                           summarize(container = sum(total_TEU, na.rm=T)) %>% 
...
sumstat_controls <- port_data_agg  %>% group_by(port_id) %>% mutate(treated=max(ownership_china)) %>% ungroup() %>%
                                      filter(treated==0, container!=0, 
                                             !is.na(quay_length) & !is.na(local_markets) & !is.na(foreign_markets) & !is.na(cargo_depth),
                                             port_code!="DEHAM") %>%
                                      merge(share, 
                                      ...
                                      group_by(port_id) %>%
                                      summarize(across(c("container", "share", "quay_length", "cargo_depth", "local_markets", "foreign_markets", "node.score"), list(agrate = ~ (tail(.x,n=1)-head(.x,n=1))/(tail(period,n=1)-head(period,n=1)))),
                                                across(c("container", "share", "quay_length", "cargo_depth", "local_markets", "foreign_markets", "node.score"), ~ mean(.x, na.rm=T), .names="{col}_mean")) %>%
                                      ungroup() %>% select(-port_id) %>%
                                      summarize(across(ends_with("mean"), mean, .names = "{col}"),
                                                across(ends_with("grate"),~ mean(.x,na.rm=T), .names = "{col}")) %>% 
                                      pivot_longer(cols = starts_with(c("container_", "share_", "quay_length_", "cargo_depth_", "local_markets_", "foreign_markets_", "node.score_")), 
                                                   names_to = c("vars", ".value"),
                                                   names_pattern = "(.*)_(.*)")
#treated 
sumstat_treated <- port_data_agg %>%  group_by(port_id) %>% mutate(treated=max(ownership_china))  %>%
                                      filter(treated==1, container!=0, port_code!="DEHAM") %>% 
                                       merge(share, 
                                            by.x = c("port_code", "period"), 
                                            by.y = c("port_code", "period"), 
                                            all.x = T) %>%
                                      arrange(port_id, period) %>%
                                      mutate(across(c("container","share", "quay_length", "cargo_depth", "foreign_markets", "local_markets","node.score"), ~ mean(.x, na.rm=T), .names="{col}_mean")) %>%
                                      group_by(port_id, ownership_china) %>%
                                      summarize(across(c("container","share", "quay_length", "cargo_depth", "foreign_markets", "local_markets", "node.score"), list(agrate = ~ (tail(.x,n=1)-head(.x,n=1))/(tail(period,n=1)-head(period,n=1)))),
                                                across(c("container_mean","share_mean", "quay_length_mean", "cargo_depth_mean", "foreign_markets_mean", "local_markets_mean", "node.score_mean"), ~ max(.x))) %>%
                                      ...
                                      pivot_longer(cols = starts_with(c("container_", "share_", "quay_length_", "cargo_depth_", "foreign_markets_", "local_markets_", "node.score_")), 
                                                   names_to = c("vars", ".value"),
                                                   names_pattern = "(.*)_(.*)")
...
sumstats <- sumstats[c(2,6,5,1,4,3),]
sumstats <- sumstats[sumstats$vars!="cranes",]
...
sumstats$vars[sumstats$vars == "foreign_markets"] <- "foreign markets"
sumstats$vars[sumstats$vars == "local_markets"] <- "local markets"
...
latex_code <- gsub("vars & mean.x & agrate & mean.y & agrate0 & agrate1"," & \\\\multicolumn{2}{c}{Control group [N=105]} & \\\\multicolumn{3}{c}{Treatment group [N=18]} \\\\\\\\\n \\\\cmidrule(lr){2-3} \\\\cmidrule(lr){4-6}  Var. & mean & $\\\\Delta$ & mean & $\\\\Delta$ [$t \\\\leq t^*$] & $\\\\Delta$ [$t>t^*$]", latex_code)
...
change_graph <- port_data_TEU_in %>% mutate(partner_region= ifelse(partner_region=="EX", "External", "Internal")) %>%
                     ...
                     scale_linetype_manual(name = "Legend",
                                          ...
                                          labels = c("North European Ports", "South European Ports", "Investment stock")) +
                     theme_minimal() +
...
contribution_graph <- world %>% merge(catch_up, by.x="iso_a2", by.y="sender_code", all.x=T) %>%
          ggplot() + geom_sf(aes(fill=change_SE))  +
           scale_fill_continuous(low="grey90", high="grey60", 
            ...
            labs(title="c) Throughput growth of South European Ports with external partner countries") +
...
upper_graph
ggsave(width=12, height=7, filename = "map_presentation.pdf", path = file.path(getwd(), "presentation_results"))
...
plot_grid(upper_graph,contribution_graph, ncol = 1, rel_heights = c(0.5,0.5))

ggsave(width=12, height=15, filename = "map3.pdf", path = file.path(getwd(), "figures"))
