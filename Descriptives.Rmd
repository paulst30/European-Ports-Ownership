...
european_ports <- port_data_agg %>% merge(PLCI, by.x = c("port_code", "year", "quarter"), by.y = c("port_code", "year", "quarter"), all.y=T) %>%
                                    filter(year==2020 | year==2006, !is.na(lat), !is.na(long)) %>% group_by(port, year) %>%
...
ggmap_change <- ggplot(data=world) + geom_sf() +
            ...
            annotate("text", x=-14.5, y=58, label="North", family="LM Roman 10")+
            annotate("text", x=-14.5, y=57, label="~Delta==XX",parse=T, family="LM Roman 10")+
            annotate("text", x=-14.5, y=44, label="South", family="LM Roman 10")+
            annotate("text", x=-14.5, y=43, label="~Delta==XX",parse=T, family="LM Roman 10")+
