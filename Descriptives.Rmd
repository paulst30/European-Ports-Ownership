...
control_var <- merge(wpi_data, terminal_data, by.x="port_code",by.y="port_code", all = T) %>%
               select(port_code,ships, cosco_owned_ships, cranes_fixed, cranes_mobile, cranes_floating, max_lift, cargo_depth, lat, long)
...
sumstat_controls <- port_data_agg %>% group_by(port_id) %>% mutate(treated=max(ownership_china)) %>% ungroup() %>%
                                      filter(treated==0, container!=0, !is.na(ships) & !is.na(max_lift) & !is.na(cargo_depth)) %>% arrange(port_id, period) %>%
                                      group_by(port_id) %>%
                                      summarize(across(c("container", "ships", "max_lift", "cargo_depth"), list(agrate = ~ (tail(.x,n=1)-head(.x,n=1))/(tail(period,n=1)-head(period,n=1)))),
                                                across(c("container", "ships", "max_lift", "cargo_depth"), ~ mean(.x, na.rm=T), .names="{col}_mean")) %>% 
                                      ...
                                      pivot_longer(cols = starts_with(c("container_", "ships_", "max_lift", "cargo_depth_")), 
...
sumstat_treated <- port_data_agg %>% group_by(port_id) %>% mutate(treated=max(ownership_china))  %>%
                                      ...
                                      mutate(across(c("container", "ships", "max_lift", "cargo_depth"), ~ mean(.x, na.rm=T), .names="{col}_mean")) %>%
                                      group_by(port_id, ownership_china) %>%
                                      summarize(across(c("container", "ships", "max_lift", "cargo_depth"), list(agrate = ~ (tail(.x,n=1)-head(.x,n=1))/(tail(period,n=1)-head(period,n=1)))),
                                                across(c("container_mean", "ships_mean", "max_lift_mean", "cargo_depth_mean"), ~ max(.x))) %>%
                                      ...
                                      pivot_longer(cols = starts_with(c("container_", "ships_", "max_lift_", "cargo_depth_")), 
                                                   names_to = c("vars", ".value"),
                                                   names_pattern = "(.*)_(.*)")

# number of ports for which controls are non missing
length(unique(port_data_agg$port_id[!is.na(port_data_agg$ships) & !is.na(port_data_agg$max_lift) & !is.na(port_data_agg$cargo_depth)]))
...
change_graph <- port_data_TEU_in %>% mutate(partner_region= ifelse(partner_region=="EX", "External", "Internal")) %>%
                     ...
                     xlim(c(1999,2021)) +
                     xlab("Year") + ylab("TEU (billions)") + labs(title="a) Container throughput and Chinese investment") +
                     scale_y_continuous(name = "TEU (billions)",
...
upper_graph <- plot_grid(change_graph + theme(legend.position = "none"), top_ten_graph+ theme(legend.position = "none"), change_legend, top_ten_legend, ncol = 2, rel_heights = c(.9,.1))
