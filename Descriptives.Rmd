...
Originally, we found 26 ports which were at some point at least partly owned by a Chinese company. The selected sample contains `r length(unique(port_data$port_code[port_data$group!=0]))` treated ports. 11 ports dropped out of the sample due to various reasons:
...
for (i in 1:length(treated_ports)){
  treatment_period <- unique(port_data$group[port_data$port_code==treated_ports[i]])
  treatment_time <- unique(port_data$time[port_data$period==treatment_period])
  port_name <- unique(port_data$port[port_data$port_code==treated_ports[i]])
  
  plot_list[[i]] <- port_data %>% filter(port_code==treated_ports[i] | group==0) %>% group_by(port_code, time) %>%
                                         summarize(sum = sum(container, na.rm=T),
