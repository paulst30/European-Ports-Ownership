...
graph_results$mean <- port_data_agg$mean_container[match(graph_results$id, port_data_agg$port_id)]

factor_level <- graph_results  %>% group_by(port) %>% slice(1)%>% arrange(desc(mean)) %>% pull(port)
