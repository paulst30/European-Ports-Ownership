...
results1 <- data.frame(Estimate = c("simple average", port_group_list$ports[match(as.character(agg_gs$egt), port_group_list$group)]),
                       ATT = c(agg_simple$overall.att,
                                   agg_gs$att.egt),
                      se = c(agg_simple$overall.se, agg_gs$se.egt))%>%
               mutate(pval = 2*pnorm(-abs(ATT/se)),
                      across(c("ATT","se"), ~ format(., big.mark = ",", digits = 2)),
                      ATT = case_when(pval<0.001 ~ paste0(ATT, "***"),
                                      pval>=0.001 & pval<0.01 ~ paste0(ATT, "**"),
                                      pval>=0.01 & pval<0.05 ~ paste0(ATT, "*"),
                                      pval>=0.05 ~ ATT),
                      se = gsub("\\s+", "",paste0("(",se,")"))) %>%
              pivot_longer(c("ATT","se"), names_to = "measure", values_to = "est")  %>%
              select(Estimate,measure, est)
...
results2 <- data.frame(Estimate = c("simple average",port_group_list$ports[match(as.character(agg_gs_cond$eg), port_group_list$group)]),
                       ATT = c(agg_simple_cond$overall.att,
                                   agg_gs_cond$att.egt),
                       se = c(agg_simple_cond$overall.se,agg_gs_cond$se.egt)) %>%
               mutate(pval = 2*pnorm(-abs(ATT/se)),
                      across(c("ATT","se"), ~ format(., big.mark = ",", digits = 2)),
                      ATT = case_when(pval<0.001 ~ paste0(ATT, "***"),
                                      pval>=0.001 & pval<0.01 ~ paste0(ATT, "**"),
...
results <- merge(results1, results2, by.x = c("Estimate", "measure"), by.y = c("Estimate", "measure"), all =  T, suffixes = c(".uc", ".c"))
...
ny_results1 <- data.frame(Estimate = c("simple average", port_group_list$ports[match(as.character(ny_agg_gs$egt), port_group_list$group)]),
                       ATT = c(ny_agg_simple$overall.att,
                                   ny_agg_gs$att.egt),
                      se = c(ny_agg_simple$overall.se,ny_agg_gs$se.egt)) %>%
               mutate(pval = 2*pnorm(-abs(ATT/se)),
                      across(c("ATT","se"), ~ format(., big.mark = ",", digits = 2)),
                      ATT = case_when(pval<0.001 ~ paste0(ATT, "***"),
                                      pval>=0.001 & pval<0.01 ~ paste0(ATT, "**"),
...
ny_results2 <- data.frame(Estimate = c("simple average",port_group_list$ports[match(as.character(ny_agg_gs_cond$egt), port_group_list$group)]),
                       ...
                       se = c(ny_agg_simple_cond$overall.se, ny_agg_gs_cond$se.egt)) %>%
                    mutate(pval = 2*pnorm(-abs(ATT/se)),
                      across(c("ATT","se"), ~ format(., big.mark = ",", digits = 2)),
                      ATT = case_when(pval<0.001 ~ paste0(ATT, "***"),
                                      pval>=0.001 & pval<0.01 ~ paste0(ATT, "**"),
...
ny_results <- merge(ny_results1, ny_results2, by.x = c("Estimate", "measure"), by.y = c("Estimate", "measure"), all =  T,  suffixes = c(".uc", ".c"))
#ny_results <- ny_results[c(11, 1:10),]


#bring output table together
results <- merge(results, ny_results, by=c("Estimate","measure"),  suffixes = c(".nt", ".nyt"), all = T)

```

```{r include=F}
# generate a Latex table
results <- results %>% select(starts_with("est"), Estimate) %>%
           mutate(Estimate= ifelse(duplicated(Estimate)>0,"",Estimate)) %>% 
           slice(c(19,20), c(1:18))
results_final <- rbind(results[c(1,2),],c("by group", rep("", ncol(results)-1)) ,results[3:20,]) 

# Insert multirow cell into the appropriate position in the LaTeX code
addtorow <- list()
addtorow$pos <- list(nrow(results_final),2, 3, 5 , 7 , 9, 11, 13, 15, 17, 19)
addtorow$command <- c("[2ex] \\hline \\hline \\multicolumn{5}{l}{\\shortstack[l]{\\emph{Signif. Codes: ***: 0.01, **: 0.05, *: 0.1}\\\\ First treatment period indicated in brackets after the portname.\\\\ Analytical standard errors in paranthesis.}}", 
                      "[2ex]",
                      "[1ex]",
                      "[2ex]",
                      "[2ex]",
...
latex_code <- print.xtable(xtable(results_final, caption = "Aggregation of group time average treatment effects"),
...
latex_code <- gsub("Estimate & est\\.uc\\.nt & est\\.c\\.nt & est\\.uc\\.nyt & est\\.c\\.nyt","Control group: & \\\\multicolumn{2}{c}{Never treated} & \\\\multicolumn{2}{c}{Not yet treated} \\\\\\\\\n PTA: & Uncond. & Cond. & Uncond. & Cond. ", latex_code)
latex_code <- gsub("Dunkerque\\, Le Havre\\, Marsaxlokk\\, Marseille\\, Nantes Saint Nazaire\\, Zeebrugge \\(66\\)",
                   "\\\\multirow{2}{*}{\\\\shortstack[l]{Dunkerque, Le Havre, Marsaxlokk, Marseille,\\\\\\\\ Nantes Saint Nazaire, Zeebrugge (66)} }", latex_code)
...
results_gt <- gt(results, rowname_col = "Estimate") |>
              tab_header(
                ...
                subtitle = "Dependent variable: Total throughput in thousand TEU") |>
              cols_label(
                est.uc.nt="unconditional",
                est.c.nt="conditional",
                est.uc.nyt="unconditional",
                est.c.nyt="conditional"
              ) |>
              tab_spanner(label = "Never treated",
                          columns = c("est.uc.nt", "est.c.nt")) |>
              tab_spanner(label = "Not yet treated",
                          columns = c("est.uc.nyt", "est.c.nyt")) |>
...
graph_results <- data.frame(group = attgt$group,
                           period = attgt$t,
                           att = attgt$att,
                           se = attgt$se,
                           c_value = attgt$c) %>%
                  mutate(treatment = ifelse(period < group, "pre", "post"),
                         ymin=att-se*c_value,
                         ymax=att+se*c_value)

graph_results$port <- port_group_list$ports[match(graph_results$group, port_group_list$group)]

ggplot(graph_results, aes(x = period, y = att/1000)) + 
  geom_point(aes(shape = treatment ),fill= "white", size = 1.2) +
  geom_ribbon(aes(ymin = ymin/1000, ymax = ymax/1000), fill = "lightgrey", alpha = 0.3) +
  facet_wrap(~port, ncol = 1, scales = "free_y") +
  labs(x = "Period", y = "TEU (millions)", caption = "Note: Confidence interval is based on analytical standard errors.") +
  theme_minimal() +
