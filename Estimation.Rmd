...
spec5 <- feols(container ~ ownership_china + ownership_china*s_china + operation | port^sender + sender^period,
               data = port_data_TEU)
spec6 <- feols(container ~ ownership_china  | port^sender + sender^period + reporter_code^period,
               data = port_data_TEU)
...
# generating a gt table
ny_results_gt <- gt(ny_results, rowname_col = "Estimate") |>
              tab_header(
                title = "Aggregation of group time average treatment effects",
                subtitle = "Control group: Not yet treated") |>
              cols_label(
                ATT.x="ATT",
                lower.x="[95%",
                upper.x="conf.int.]",
