...
```{r include=FALSE, warning=FALSE, message=FALSE}
# include Sea dummies: Mediterranean Sea (45) [24 Eastern Mediterranean, 21 Western Mediterranean], Atlantic Ocean (36), North Sea (37), Baltic Sea (19), Black Sea (3)

## (1) Mediterranean Sea ports: 1 for all ports in countries at the Mediterranean Sea

med_sea_countries <- c("Cyprus", "Greece", "Croatia", "Italy", 
                       "Montenegro", "Malta", "Slovenia", "Turkey", "Spain", "France")

port_data_agg <- port_data_agg %>%
  mutate(Med_Sea = ifelse(reporter %in% med_sea_countries, 1, 0))

### Replace with 0 for ports in Turkey, Spain, France that are not at the Mediterranean Sea

port_data_agg <- port_data_agg %>%
  mutate(Med_Sea = ifelse(port == "Samsun", 0, Med_Sea))

non_med_ports_spanish <- c("Bilbao", "Gijon", "Huelva", "Las Palmas", 
                   "Marin y ria de Pontevedra", "Santa Cruz de Tenerife", "Las Nieves (Agaete)", "Cadiz",
                   "Santander", "Sevilla", "Vigo", "Villagarcia")

port_data_agg <- port_data_agg %>%
  mutate(Med_Sea = ifelse(reporter == "Spain" & port %in% non_med_ports_spanish, 0, Med_Sea))

non_med_ports_french <- c("France: Guadeloupe and Martinique", 
                           "Brest", "Dunkerque", "Le Havre", 
                           "Nantes Saint Nazaire", "Pointe a Pitre (Guadeloupe)", 
                           "Fort de France (Martinique)", 
                           "Port Réunion (ex Pointe-des-Galets) (Réunion)")

port_data_agg <- port_data_agg %>%
  mutate(Med_Sea = ifelse(reporter == "France" & port %in% non_med_ports_french, 0, Med_Sea))

med_sea_ports <- port_data_agg %>%
  filter(Med_Sea == 1) %>%
  distinct(port) %>%
  pull(port)

print(med_sea_ports)
### 45 of our ports are in the greater space of the Mediterranean Sea and 93 are not, total ports = 140, therefore 1/3 of the sample ports are in the Mediterranean.

## (2) Atlantic Ocean
atlantic_ports <- c("Bilbao", "Gijon", "Huelva", "Las Palmas", 
                    "Marin y ria de Pontevedra", "Santa Cruz de Tenerife", 
                    "Santander", "Sevilla", "Vigo", "Villagarcia", "Las Nieves (Agaete)",
                    "Brest", "Dunkerque", "Le Havre", "Nantes Saint Nazaire", "France:                      Guadeloupe and Martinique", "Canical", "Funchal (Madeira)", "Fort de                     France (Martinique)", "Belfast", "Bristol", "Cardiff", "Portsmouth", 
                    "Southampton", "Warrenpoint", "Drogheda", "Cadiz",
                    "Dublin", "Cork", "Waterford", "Leixoes", 
                    "Lisboa", "Ponta Delgada (Ilha de S. Miguel, Acores)", 
                    "Praia da Vitoria (Ilha Terceira, Acores)", "Setubal", 
                    "Sines")

port_data_agg <- port_data_agg %>%
  mutate(Atlantic_Ocean = ifelse(port %in% atlantic_ports, 1, 0))
### 36 ports are in the Atlantic Ocean or connect to it.

## (3) North Sea
north_sea_ports <- c("Antwerp-Bruges", "Antwerpen", "Gent (Ghent)", "Zeebrugge", 
                     "Bremerhaven", "Cuxhaven", "Hamburg", "Kiel", "Lubeck", 
                     "Wilhelmshaven", "Aalborg", "Arhus", "Borg", "Kobenhavns Havn", 
                     "Esbjerg", "Fredericia (Og Shell-Havnen)", "Amsterdam", 
                     "Moerdijk", "Rotterdam", "Aberdeen", "Felixstowe", 
                     "Goole", "Hull", "Immingham", "London", "Medway", 
                     "Tees & Hartlepool", "Fredrikstad", "Porsgrunn, Rafnes, Herøya, Brevik, Skien, Langesund, Voldsfjorden", "Kristiansand", "Mosjoen", 
                     "Moss", "Oslo", "Goteborg", "Halmstad", "Helsingborg", "Malmo")

port_data_agg <- port_data_agg %>%
  mutate(North_Sea = ifelse(port %in% north_sea_ports, 1, 0))
### 37 ports are in the North Sea or connect to it.

## (4) Baltic Sea
baltic_ports <- c("Tallinn", "Helsinki", "Hanko", "Oulu", "Pori",
                   "Rauma", "Klaipeda", "Riga", "Gdansk", "Gdynia", "Szczecin",
                   "Gavle", "Halmstad", "Helsingborg", "Malmo", "Norrkoping",
                   "Sodertalje", "Vasteras", "Gavle")

port_data_agg <- port_data_agg %>%
  mutate(Baltic_Sea = ifelse(port %in% baltic_ports, 1, 0))
# 19 ports are in the Baltic Sea or connect to it.

## (5) Black Sea
black_sea_ports <- c("Varna", "Constanta", "Samsun")

port_data_agg <- port_data_agg %>%
  mutate(Black_Sea = ifelse(port %in% black_sea_ports, 1, 0))
### 3 ports are in the Black Sea.

## (5) splitting up the Mediterranean in Eastern and Western Mediterranean Sea

eastern_ports <- c(
  "Lemesos (Limassol)", "Piraeus", "Thessaloniki", "Ploce", "Rijeka", "Ancona", "Bari", "Catania", "Ravenna", "Taranto", "Trieste", "Venezia", "Bar", "Marsaxlokk", "Koper", "Aliaga",
  "Ambarli", "Antalya", "Gemlik", "Iskenderun, Hatay", "Izmir", "Izmit", "Mersin", "Tekirdag"
)

western_ports <- c(
  "Malaga", "Alicante", "Algeciras", "Barcelona", "Cartagena",
  "Castellon", "Melilla", "Gioia Tauro", "Palma de Mallorca", "Tarragona", "Valencia", "Marseille", "Cagliari",
  "Civitavecchia", "Genova", "Livorno", "Marina Di Carrara", "Napoli", "Salerno", "La Spezia", "Savona"
)

port_data_agg$Med_Sea_east <- ifelse(port_data_agg$port %in% eastern_ports, 1, 0)
port_data_agg$Med_Sea_west <- ifelse(port_data_agg$port %in% western_ports, 1, 0)

### 24 ports are in the Eastern Mediterranean and 21 ports are in the Western Mediterranean.

# Create a categorial variable. Sea1 captures all Mediterranean ports, Sea2 differentiated between Eastern and Western Mediterranean ports

port_data_agg <- port_data_agg %>%
  mutate(Sea1 = case_when(
    Med_Sea == 1 ~ "Med_Sea",
    Atlantic_Ocean == 1 ~ "Atlantic_Ocean",
    North_Sea == 1 ~ "North_Sea",
    Baltic_Sea == 1 ~ "Balti_Sea",
    Black_Sea == 1 ~ "Black_Sea"
  ))

port_data_agg$Sea1 <- as.factor(port_data_agg$Sea1)

port_data_agg <- port_data_agg %>%
  mutate(Sea2 = case_when(
    Med_Sea_east == 1 ~ "Med_Sea_east",
    Med_Sea_west == 1 ~ "Med_Sea_west",
    Atlantic_Ocean == 1 ~ "Atlantic_Ocean",
    North_Sea == 1 ~ "North_Sea",
    Baltic_Sea == 1 ~ "Balti_Sea",
    Black_Sea == 1 ~ "Black_Sea"
...
port_data_agg$Sea2 <- as.factor(port_data_agg$Sea2)

# generate interaction-terms with main controls to implement regional control groups

port_data_agg <- port_data_agg %>% mutate(across(c("Med_Sea", "Atlantic_Ocean", "North_Sea", "Baltic_Sea", "Black_Sea"),list(quay=~.*quay_length, lift=~.*max_lift, depth=~.*cargo_depth), .names = "{.col}_{.fn}"  ))

regional_controls <- c("Med_Sea", "Atlantic_Ocean", "North_Sea", "Baltic_Sea", "Black_Sea", 
                       "Med_Sea_quay", "Atlantic_Ocean_quay", "North_Sea_quay", "Baltic_Sea_quay", "Black_Sea_quay",
                       "Med_Sea_lift", "Atlantic_Ocean_lift", "North_Sea_lift", "Baltic_Sea_lift", "Black_Sea_lift",
                       "Med_Sea_depth", "Atlantic_Ocean_depth", "North_Sea_depth", "Baltic_Sea_depth", "Black_Sea_depth")
...
# Save modified LaTeX code to a .tex file
cat(latex_code, file = file.path(getwd(), "results_tables", "did_results_presentation.tex"))
...
factor_level <- c("Rotterdam", "Antwerpen", "Valencia", "Piraeus", "Barcelona", "Le Havre","Ambarli", "Genova", "Marseille", "Zeebrugge", "Bilbao", "Gdynia", "Thessaloniki", "Dunkerque", "Nantes Saint Nazaire", "Moerdijk", "Amsterdam", "Marsaxlokk", "Savona")
...
# robustness check for estimating treatment effect in a regional comparison
attgt_cond_ny_regional <- simple_staggered_did(yname = "container",            #outcome variable
                              tname = "period",                 #time variable
                              idname = "port_id",                    #id for units (should be port_code*sender)
                              gname = "group",                    #grouping variable for first treatment. Is zero for never treated units)
                              xformula = regional_controls,
                              varformula = var_controls,
                              control_group = "not_yet_treated",
                              bstrap = 400,
                              data = port_data_agg)
...
# get results
results_ny_reg <- data.frame(Estimate = c("simple average", port_list$port[match(as.character(attgt_cond_ny_regional$groupwise_treatment_effects$id), port_list$id)]),
                       ATT_cond_ny = c(attgt_cond_ny_regional$average_treatment_effect$att,
                                   attgt_cond_ny_regional$groupwise_treatment_effects$att),
                       lower_cond_ny = c(attgt_cond_ny_regional$average_treatment_effect$att - attgt_cond_ny_regional$average_treatment_effect$crit_val,
                                attgt_cond_ny_regional$groupwise_treatment_effects$att - attgt_cond_ny_regional$groupwise_treatment_effects$crit_val),
                       upper_cond_ny = c(attgt_cond_ny_regional$average_treatment_effect$att + attgt_cond_ny_regional$average_treatment_effect$crit_val,
                                  attgt_cond_ny_regional$groupwise_treatment_effects$att + attgt_cond_ny_regional$groupwise_treatment_effects$crit_val),
                       p_value_cond_ny = c(NA, attgt_cond_ny_regional$groupwise_treatment_effects$pre_treatment_p_value)) %>%
               mutate(sig_cond = (ATT_cond_ny>0 & lower_cond_ny>0) | (ATT_cond_ny<0 & upper_cond_ny<0),
                      pretest_cond = (p_value_cond_ny < 0.05) & !is.na(p_value_cond_ny),
                      across(c("ATT_cond_ny","lower_cond_ny","upper_cond_ny"), ~ format(round(.,digits=0), big.mark = ",")),
                      ATT_cond_ny = ifelse(sig_cond==T, paste0(ATT_cond_ny, "*"), ATT_cond_ny),
                      ATT_cond_ny = ifelse(pretest_cond==T, paste0(ATT_cond_ny, "^{x}"),ATT_cond_ny),
                      Conf_cond_ny = paste0("[",str_trim(lower_cond_ny),"; ",str_trim(upper_cond_ny),"]"))


results_ny_reg_final <- rbind(results_ny_reg[1,],c("by group", rep("", ncol(results_ny_reg)-1)) ,results_ny_reg[2:19,]) 

# generating a gt table
results_ny_reg_gt <- gt(results_ny_reg[c("Estimate","ATT_cond_ny","Conf_cond_ny")], rowname_col = "Estimate") |>
              tab_header(
                title = "Aggregation of group time treatment effects with regional control groups",
                subtitle = "Dependent variable: Total throughput in thousand TEU") |>
              cols_label(
                ATT_cond_ny="conditional",
                Conf_cond_ny="95% CI"
...
#manually putting together eventstudy graph 
graph_results <- attgt_cond_ny_regional$grouptime_treatment_effects %>%
                  filter(g!=106) %>%                            #drop Hamburg due to no treatment effects
                  mutate(treatment = ifelse(t < g, "pre", "post"),
                         ymin=att-uniform_crit_val,
                         ymax=att+uniform_crit_val)
...
ggplot(graph_results, aes(x = t, y = att/1000)) + 
  ...
  geom_ribbon(aes(ymin = ymin/1000, ymax = ymax/1000), fill = "lightgrey", alpha = 0.3) +
  facet_wrap(~port, ncol = 2, scales = "free_y") +
  labs(x = "Period", y = "TEU (thousands)", caption = "Note: Uniform confidence interval is based on bootstrapped errors.") +
  theme_minimal() +
  theme(panel.background = element_blank(),
        strip.text = element_text(hjust = 0, size=12),
        text = element_text(size=10, family="LM Roman 10"),
