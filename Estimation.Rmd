...
port_data_agg <- port_data %>% mutate(origin_china = ifelse(!is.na(container) & (sender_code=="CN_X_HK" | sender_code=="HK"),container,0)) %>%
                               group_by(port, port_code, period) %>% 
...
spec4 <- feols(container ~ ownership_china + ownership_china*s_china | port^sender + sender^period,
               data = port_data)
spec5 <- feols(container ~ ownership_china + ownership_china*s_china + operation | port^sender + sender^period,
               data = port_data)

fe_results<- etable(spec1, spec2, spec3, spec4, spec5,
...
attgt_op <- att_gt(yname = "container",            #outcome variable
                tname = "period",                 #time variable
                idname = "port_id",                    #id for units (should be port_code*sender)
                gname = "group_operation",                    #grouping variable for first treatment. Is zero for never treated units
                #xformla = ~X,                  #for potential covariates
                #control_group="notyettreated",  #use not yet treated units as control group in addition to never treated units
                allow_unbalanced_panel = TRUE,  #allows he use of unbalanced panel data. Increases computation time.
                #est_method = "reg",             #indicates which estimation method should be used ("ipw", inverse probability weighting or "reg" for regression)
...
results2 <- data.frame(Estimate = c("simple average",port_group_list$ports[match(as.character(agg_gs_op$eg), port_group_list$group)]),
                       ATT = c(agg_simple_op$overall.att,
                                   agg_gs_op$att.egt),
                      lower = c(agg_simple_op$overall.att-(agg_simple_op$overall.se*1.95), agg_gs_op$att.egt-(agg_gs_op$se.egt*agg_gs_op$crit.val.egt)),
                      upper = c(agg_simple_op$overall.att+(agg_simple_op$overall.se*1.95), agg_gs_op$att.egt+(agg_gs_op$se.egt*agg_gs_op$crit.val.egt)))
results <- merge(results1, results2, by.x = "Estimate", by.y = "Estimate", all =  T)
...
results_gt <- gt(results, rowname_col = "Estimate") |>
              ...
              tab_spanner(label = "Chinese ownership",
              ...
              tab_footnote(footnote = "Confidence interval does not include zero.",
                           location = list(cells_body(columns=c(ATT.x),
...
ny_attgt_op <- att_gt(yname = "container",            #outcome variable
                tname = "period",                 #time variable
                idname = "port_id",                    #id for units (should be port_code*sender)
                gname = "group_operation",                    #grouping variable for first treatment. Is zero for never treated units
                #xformla = ~X,                  #for potential covariates
...
ny_agg_simple_op <- aggte(ny_attgt_op, type = "simple", na.rm=T)

# option 3: groupwise treatment effects
ny_agg_gs <- aggte(ny_attgt, type = "group", na.rm=T)
ny_agg_gs_op <- aggte(ny_attgt_op, type = "group", na.rm=T)
...
ny_results2 <- data.frame(Estimate = c("simple average",port_group_list$ports[match(as.character(ny_agg_gs_op$egt), port_group_list$group)]),
                       ATT = c(ny_agg_simple_op$overall.att,
                                   ny_agg_gs_op$att.egt),
                      lower = c(ny_agg_simple_op$overall.att-(ny_agg_simple_op$overall.se*1.95), ny_agg_gs_op$att.egt-(ny_agg_gs_op$se.egt*ny_agg_gs_op$crit.val.egt)),
                      upper = c(ny_agg_simple_op$overall.att+(ny_agg_simple_op$overall.se*1.95), ny_agg_gs_op$att.egt+(ny_agg_gs_op$se.egt*ny_agg_gs_op$crit.val.egt)))
ny_results <- merge(ny_results1, ny_results2, by.x = "Estimate", by.y = "Estimate", all =  T)
ny_results <- results[c(11, 1:10),]



# generating a gt table
ny_results_gt <- gt(results, rowname_col = "Estimate") |>
              tab_header(
                title = "Aggregation of group time average treatment effects",
                subtitle = "Control group: Never treated and not yet treated") |>
              cols_label(
                ATT.x="ATT",
                lower.x="[95%",
                upper.x="conf.int.]",
...
ggdid(agg_es) + scale_x_continuous(breaks = seq(-80, 80, 10)) + labs(title = "Average Effect by Length of Exposure of Ports with Chinese Ownership")
ggdid(agg_es_op) + scale_x_continuous(breaks = seq(-80, 80, 10)) + labs(title = "Average Effect by Length of Exposure of Ports with Chinese Operator")
...
attgt_share <- att_gt(yname = "share_china",            #outcome variable
                tname = "period",                 #time variable
                idname = "port_id",                    #id for units (should be port_code*sender)
                gname = "group",                    #grouping variable for first treatment. Is zero for never treated units
                #xformla = ~X,                  #for potential covariates
...
attgt_op_share <- att_gt(yname = "share_china",            #outcome variable
                tname = "period",                 #time variable
                idname = "port_id",                    #id for units (should be port_code*sender)
                gname = "group_operation",                    #grouping variable for first treatment. Is zero for never treated units
                #xformla = ~X,                  #for potential covariates
...
agg_simple_op_share <- aggte(attgt_op_share, type = "simple", na.rm=T)

# option 2: event-study type aggregation by time under treatment
agg_es_share <- aggte(attgt_share, type = "dynamic", na.rm=T)
agg_es_op_share <- aggte(attgt_op_share, type = "dynamic", na.rm=T)

# option 3: groupwise treatment effects
agg_gs_share <- aggte(attgt_share, type = "group", na.rm=T)
agg_gs_op_share <- aggte(attgt_op_share, type = "group", na.rm=T)
...
share_results <- share_results[c(11, 1:10),]
