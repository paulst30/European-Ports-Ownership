...
for (i in 1:length(time)) {                                                   # loop over all reporter
  ...
  if (i==1){
    port_data_TEU_in <- x                                                                  # save and append to previous tables
  } else {
    port_data_TEU_in <- rbind(port_data_TEU_in, x)
  }
}



port_data_TEU_in <- pivot_wider(port_data_TEU_in, values_from = "TEU", names_from = "loadstat") %>%
...
for (i in 1:nrow(missing_entries2)) {
  port_data_TEU_in$port[port_data_TEU_in$rep_mar==missing_entries2$rep_mar[i]] <- missing_entries2$port[i]
  port_data_TEU_in$aggregate[port_data_TEU_in$rep_mar==missing_entries2$rep_mar[i]] <- missing_entries2$aggregate[i]
  port_data_TEU_in$stat_port[port_data_TEU_in$rep_mar==missing_entries2$rep_mar[i]] <- missing_entries2$stat_port[i]
  port_data_TEU_in$port_code[port_data_TEU_in$rep_mar==missing_entries2$rep_mar[i]] <- missing_entries2$rep_mar[i]
}
...
for (i in 1:nrow(incorrect_aggregates)) {
  port_data_TEU_in$aggregate[port_data_TEU_in$port_code==incorrect_aggregates$port_code[i]] <- incorrect_aggregates$aggregate[i]
  port_data_TEU_in$stat_port[port_data_TEU_in$port_code==incorrect_aggregates$port_code[i]] <- incorrect_aggregates$stat_port[i]
}

save(list = "port_data_TEU_in", file = "downloaded_data_TEU_in.RData")  
...
port_data_TEU_in <- merge(port_data_TEU_in, gdp_data, by.x = c("reporter_code", "year", "quarter"), by.y = c("geo", "year", "quarter"), all.x = TRUE)

######## ADD INFOMATION ON PORT GROUPS ########
average_TEU <- port_data_TEU_in %>% group_by(port_code, year) %>% summarize(TEU_yrly=sum(total_TEU, na.rm = T)) %>% ungroup() %>%
...
port_data_TEU_in <- merge(port_data_TEU_in, average_TEU, by.x = "port_code", by.y = "port_code", all.x = T)
...
port_data_TEU_in <- merge(port_data_TEU_in, treatment, by.x = c("port_code", "year", "quarter"), by.y = c("port_code", "year", "quarter"), all.x = T) %>% 
...
port_data_TEU_in$operation[port_data_TEU_in$port=="Rotterdam" & port_data_TEU_in$year<2016] <- 0 # Operations in Rotterdam were taken over since 2016
port_data_TEU_in$group_operation[port_data_TEU_in$port=="Rotterdam"] <- 80  # operation started Sept. 2016 

port_data_TEU_in <- port_data_TEU_in[!is.na(port_data_TEU_in$port),]       #delete remaining observations that are not associated with an port

######## SAVE ############

save(list = "port_data_TEU_in", file = "port_data_TEU_in.RData")                                # save in current directory

rm(list = setdiff(ls(), "port_data_TEU_in"))                                             #delete all helper objects
