...
robustness_cosco <- terminal_data %>% filter(!is.na(terminal_TEU)) %>% merge(read.csv("WPI_data.csv"), by.x = c("port_code"), by.y=c("port_code"), all.x=T) %>% select(port_code, port, year, quarter, period, terminal_TEU, cosco_owned_ships, max_lift, cargo_depth) %>% rename(ships=cosco_owned_ships,
...
#control ports
control_ports <- port_data_agg %>% filter(group==0) %>% merge(terminal_number, by.x="port_code", by.y = "port_code", all.x=T)  

# putting it all together
robustness_cosco <- rbind(robustness_cosco, control_ports[, colnames(robustness_cosco)])%>% group_by(port_code) %>%
                    mutate(mean_container=mean(container,na.rm=T),
...
addtorow$command <- c("[2ex] \\hline \\hline \\multicolumn{3}{l}{\\shortstack[l]{Stars indicate that the uniform confidence interval \\\\does not include zero.\\\\}}", 
                      "[1ex]")
