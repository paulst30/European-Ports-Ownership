...
results <- data.frame(Estimate = c("simple average", port_group_list_rob$ports[match(as.character(agg_gs_rob$egt), port_group_list_rob$group)]),
                       ATT = c(agg_simple_rob$overall.att,
                                   agg_gs_rob$att.egt),
                       se = c(agg_simple_rob$overall.se,agg_gs_rob$se.egt)) %>%
...
results_gt_rob <- gt(results, rowname_col = "Estimate") |>
              ...
              sub_missing(columns = everything(),
                          rows = everything(),
                          missing_text = "") |>
              tab_footnote(footnote = "Signif. Codes: ***: 0.001, **: 0.01, *: 0.05.") |>
              tab_footnote(footnote = "Analytical standard errors in parentheses.")
...
results <- data.frame(Estimate = c("simple average", port_group_list$ports[match(as.character(agg_gs_woCHN$egt), port_group_list$group)]),
                      ...
                      se = c(agg_simple_woCHN$overall.se, agg_gs_woCHN$se.egt)) %>%
               mutate(pval = 2*pnorm(-abs(ATT/se)),
                      across(c("ATT","se"), ~ format(., big.mark = ",", digits = 2)),
                      ATT = case_when(pval<0.001 ~ paste0(ATT, "***"),
                                      pval>=0.001 & pval<0.01 ~ paste0(ATT, "**"),
                                      ...
                                      pval>=0.05 ~ ATT),
                      se = gsub("\\s+", "",paste0("(",se,")"))) %>%
              pivot_longer(c("ATT","se"), names_to = "measure", values_to = "est") %>% select(est, Estimate, measure)  
...
#results
share_results <- data.frame(Estimate = c("simple average", port_group_list$ports[match(as.character(agg_gs_share$egt), port_group_list$group)]),
                       ATT = c(agg_simple_share$overall.att,
                                   agg_gs_share$att.egt),
                       se = c(agg_simple_share$overall.se, agg_gs_share$se.egt)) %>%
               mutate(pval = 2*pnorm(-abs(ATT/se)),
                      across(c("ATT","se"), ~ format(., big.mark = ",", digits = 2)),
                      ATT = case_when(pval<0.001 ~ paste0(ATT, "***"),
                                      pval>=0.001 & pval<0.01 ~ paste0(ATT, "**"),
...
# combining results
results <- merge(results, share_results, by = c("Estimate", "measure")) %>%
           mutate(Estimate= ifelse(duplicated(Estimate)>0,"",Estimate)) %>% 
           slice(c(21,22), c(1:20))

results_final <- rbind(results[c(1,2),],c("by group", rep("", ncol(results)-1)) ,results[3:22,]) 
...
results_gt_woCHN <- gt(results, rowname_col = "Estimate") |>
              ...
              cols_label(
                est.x="TEU without China",
                est.y="Share of chinese TEU") |>
