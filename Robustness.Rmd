...
# combining results
results <- merge(results, share_results, by = "Estimate")
results <- results[c(11,seq(1,10,1)),]
...
results_gt_woCHN <- gt(results, rowname_col = "Estimate") |>
              ...
              cols_label(
                ATT.x="TEU without China",
                lower.x="[95%",
                upper.x="conf.int.]",
                ATT.y="Share of chinese TEU",
                lower.y="[95%",
                upper.y="conf.int.]") |>
              ...
              tab_row_group(label = "by group:",
                            rows = contains("(")) |>
              row_group_order(groups = c(NA,"by group:")) |>
              cols_hide(c("lower.x","upper.x", "lower.y","upper.y")) |>
              sub_missing(columns = everything(),
                          rows = everything(),
                          missing_text = "") |>
              tab_footnote(footnote = "Confidence interval does not include zero.",
                           location = list(cells_body(columns= ATT.x,
                                                 rows = lower.x > 0 | upper.x < 0),
                                           cells_body(columns=ATT.y,
